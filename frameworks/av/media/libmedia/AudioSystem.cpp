...
status_t AudioSystem::setParameters(audio_io_handle_t ioHandle, const String8& keyValuePairs) {
    ...
    {  
        ...
        if(aps != 0)
        {
            ...
            if(value != 0)
            {
                aps->StartOutputSamplerate(output, AUDIO_STREAM_MUSIC,0,44100);
            }
            else
            {
                aps->StopOutputSamplerate(output, AUDIO_STREAM_MUSIC,0,44100);
            }
        }
    }
    ...
    {  
        ...
        if(aps != 0)
        {
            ...
            if(value != 0)
            {
                aps->startOutput(output,AUDIO_STREAM_MUSIC);
            }
            else
            {
                aps->stopOutput(output,AUDIO_STREAM_MUSIC);
            }
        }
    }
#endif
...
status_t AudioSystem::StartOutputSamplerate(audio_io_handle_t output, audio_stream_type_t stream, int session , int samplerate)
{
#ifdef MTK_AUDIO
    const sp<IAudioPolicyService> &aps = AudioSystem::get_audio_policy_service();
    if (aps == 0) { return false; }
...
status_t AudioSystem::StopOutputSamplerate(audio_io_handle_t output, audio_stream_type_t stream, int session , int samplerate)
{
#ifdef MTK_AUDIO
    const sp<IAudioPolicyService> &aps = AudioSystem::get_audio_policy_service();
    if (aps == 0) { return false; }
