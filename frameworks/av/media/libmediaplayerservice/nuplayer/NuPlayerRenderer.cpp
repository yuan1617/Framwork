...
void NuPlayer::Renderer::queueEOS(bool audio, status_t finalResult) {
    ...
    if (mFlags & FLAG_HAS_VIDEO_AUDIO) {
        mFlags &= ~FLAG_HAS_VIDEO_AUDIO;
    }
	mSyncQueues = false; //avoid Renderer is blocked in sync queue
#endif 
...
#ifdef MTK_SLOW_MOTION_VIDEO_SUPPORT
	if(mSMSpeed != 0){
		positionUs = (nowUs - mAnchorTimeRealUs)/mSMSpeed + mAnchorTimeMediaUs;
	}
#endif	
#endif
...
#ifdef MTK_SLOW_MOTION_VIDEO_SUPPORT		
		if((mSMSpeed != 0)&&(mSMSpeed != 1)){
			positionUs = mAnchorTimeMediaUs - mAnchorTimeRealUs+ mPauseStartedTimeRealUs;
		}
#endif
...
int64_t  NuPlayer::Renderer::getPlayedOutAudioDurationUsByAudioTrackCenter() {
		uint32_t numFramesPlayed;
		numFramesPlayed = getNumFramesPlayedByAudioTrackCenter();
		return  numFramesPlayed * mAudioSink->msecsPerFrame() * 1000;
...
bool NuPlayer::Renderer::handleRenderBufferLateInfo(bool tooLate,int64_t realTimeUs,QueueEntry *processBufferEntry){
    ...
    ALOGD("[%s buffer] ACodec delay time(%lld us), video mediaTimeUs(%lld us), mAnchorTimeMediaUs(%lld us),mAnchorTimeRealUs(%lld us) ", 
		(mVideoLateByUs > 0)?"late":"early",-mVideoLateByUs, mediaTimeUs, mAnchorTimeMediaUs,mAnchorTimeRealUs);
   return tooLate;
...
void NuPlayer::Renderer::handleRenderBufferForSlowMotion(QueueEntry *processBufferEntry){
	int64_t mediaTimeUs; 
	CHECK(processBufferEntry->mBuffer->meta()->findInt64("timeUs", &mediaTimeUs));
	if((mSpeedChanged)&& !mHasAudio){
		mAnchorTimeMediaUs = mediaTimeUs;		
		mAnchorTimeRealUs = ALooper::GetNowUs();
		mSpeedChanged = false;
	}
	if((mSMSpeed >0)&&!mHasAudio){
