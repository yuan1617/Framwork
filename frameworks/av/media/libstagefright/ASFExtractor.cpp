            ...
            if (mType == ASF_VIDEO) {
                ...
                while (!bIsKeyFrame) {
                    ...
                    if (ASF_END_OF_FILE == retVal) {
                        ...
                        if( (diff > 500000 || diff < -500000) && repeat_cnt < 10) {
...
// ]
    ...
    *out = buffer;
    ...
    ASF_DEBUG_LOGV(
            "GetNextMediaFrame StreamId[%d], ts=%d, IsKey=%d, replidata_size=%d, (*out)->range_length()=%d\n",
            mTracks.editItemAt(CurTrackIndex).mTrackNum, timestamp,
            bIsKeyFrame, replicatedata_size, (*out)->range_length());
    ...
    if (mAsfParser != NULL) {
        ...
        do {
            ...
            if (!pNextPacket->payload_count) {
                int ret = mAsfParser->asf_get_stream_packet(pNextPacket,
                        mTracks.editItemAt(CurTrackIndex).mTrackNum);
                ASF_DEBUG_LOGV(
                        "[ASF]:GetNextMediaPayload 1: CurTrackIndex = %d, find a new packet, contain payloads = %d \n",
...
status_t ASFExtractor::addMPEG4CodecSpecificData(const sp<MetaData>& mMeta) {
    ...
    ALOGI("vopStart = %u,vopEnd = %u", vopStart,vopEnd); //lu% to %u, zyx added
