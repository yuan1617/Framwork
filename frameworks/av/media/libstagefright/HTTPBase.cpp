...
#ifdef MTK_AOSP_ENHANCEMENT
    if (++mNumBandwidthHistoryItems > 20) {//about 20 / 2 *5 s for DASH OR 20 *3 for HLS
#else
...
#ifdef MTK_AOSP_ENHANCEMENT
bool HTTPBase::estimateBandwidth(int32_t *bps_longterm,int32_t *bps_shortterm)
{
    Mutex::Autolock autoLock(mLock);
    ...
    *bps_longterm = ((double)mTotalTransferBytes * 8E6 / mTotalTransferTimeUs);
    ...
    {
        if(i >= (count - 2))//the lastest two entries
        {
    ...
    *bps_shortterm = ((double)numBytes * 8E6 / delayUs);
