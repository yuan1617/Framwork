        ...
        case kWhatCodecNotify:
        {
            ...
            switch (what) {
                ...
                case CodecBase::kWhatComponentAllocated:
                {
	                ...
	                MC_LOGI("[what=kWhatComponentAllocated][msg='%s' ]", msg->debugString().c_str());
                ...
                {
                    MC_LOGI("[what=kWhatComponentConfigured][msg='%s' ]", msg->debugString().c_str());
                ...
                {
                    MC_LOGI("[what=kWhatInputSurfaceCreated]");
                ...
                {
                    MC_LOGI("[what=kWhatSignaledInputEOS]");
                ...
                case CodecBase::kWhatBuffersAllocated:
                {
	                ...
	                MC_LOGI("[what=kWhatBuffersAllocated][msg='%s' ]",msg->debugString().c_str());
	    ...
	    if (!(omxFlags & OMX_BUFFERFLAG_DUMMY_NALU)) {
		 ...
		 if (mFlags & kFlagGatherCodecSpecificData) {
                ...
                case CodecBase::kWhatShutdownCompleted:
                {
	                ...
	                MC_LOGI("[what=kWhatShutdownCompleted][mState=%d][msg='%s' ]",mState,msg->debugString().c_str());
                    ...
                    if (mState == STOPPING) {
                        ...
                        if (mState!=UNINITIALIZED) { // prevent release twice
                            ...
                            MC_LOGI("[what=kWhatShutdownCompleted] Warning, no need release again, check upper flow");
                        }
#endif
                    }
                    mFlags &= ~kFlagIsComponentAllocated;
                ...
                {
										MC_LOGI("[what=kWhatFlushCompleted][mState=%d][msg='%s' ]",mState,msg->debugString().c_str());
                    if (mState != FLUSHING) {
...
status_t MediaCodec::queueCSDInputBuffer(size_t bufferIndex) {
    ...
    if (csd->size() > codecInputData->capacity()) {
	    ...
	    MC_LOGI("[error] csd->size()  %d  > codecInputData->capacity() %d", csd->size() , codecInputData->capacity());
...
status_t MediaCodec::onQueueInputBuffer(const sp<AMessage> &msg) {
    ...
    if (msg->findSize("size", &size)) {
        ...
        if (mCrypto == NULL) {
	        ...
	        MC_LOGI("[error VAL] mCrypto == NULL" );
    ...
    if (index >= mPortBuffers[kPortIndexInput].size()) {
	    ...
	    MC_LOGI("[error RANGE]index %d > mPortBuffers[%d].size() %d",
    ...
    if (info->mNotify == NULL || !info->mOwnedByClient) {
	    ...
	    MC_LOGI("[error access]%d,%d",(info->mNotify == NULL),(!info->mOwnedByClient));

        return -EACCES;
    }

    if (offset + size > info->mData->capacity()) {
	    MC_LOGI("[error VAL] offset=%d , size %d >info->mEncryptedData->capacity() %d  ",offset,size,info->mEncryptedData->capacity());
    ...
    if (mCrypto != NULL) {
        if (size > info->mEncryptedData->capacity()) {
	        ...
	        MC_LOGI("[error RANGE] size %d >info->mEncryptedData->capacity() %d  ",size,info->mEncryptedData->capacity());
...
status_t MediaCodec::onReleaseOutputBuffer(const sp<AMessage> &msg) {
    ...
    if (!msg->findInt32("render", &render)) {
		...
		if (!isExecuting()) {
	       MC_LOGI("[error] mState != isExecuting");
        return -EINVAL;
    }

    if (index >= mPortBuffers[kPortIndexOutput].size()) {
	    MC_LOGI("[error]index >size  ");
        return -ERANGE;
    }
    ...
    if (info->mNotify == NULL || !info->mOwnedByClient) {
	    ...
	    MC_LOGI("[error]%d,%d", (info->mNotify == NULL),(!info->mOwnedByClient));
...
ssize_t MediaCodec::dequeuePortBuffer(int32_t portIndex) {
    ...
    if (availBuffers->empty()) {
	    ...
	    //MC_LOGI("[%s]portIndex=%d no buffer avail",__FUNCTION__,portIndex);
