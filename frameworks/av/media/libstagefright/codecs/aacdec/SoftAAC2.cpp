    ...
    while ((!inQueue.empty() || mEndOfInput) && !outQueue.empty()) {
        ...
        if (!inQueue.empty()) {
            ...
            if (inHeader->nFilledLen == 0 && 
				(getBytesInDecoder() == 0 || mEndOfInput || nDecodingNotEnoughBits)) {
#else
            if (inHeader->nFilledLen == 0) {
#endif
                inInfo->mOwnedByUs = false;
...
#ifdef MTK_AOSP_ENHANCEMENT
			if ((getBytesInDecoder() == 0 || mEndOfInput || nDecodingNotEnoughBits) && 
				inBufferLength[0] > 0) {
            	// Fill and decode
...
#ifdef MTK_AOSP_ENHANCEMENT
			numSamples = mStreamInfo->frameSize * mStreamInfo->numChannels;
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT
			int lastAvailInRingBuffer = outputDelayRingBufferSamplesAvailable();
 			if (!outQueue.empty() && lastAvailInRingBuffer < mStreamInfo->frameSize * mStreamInfo->numChannels) { 
#else
            if (mEndOfInput && !outQueue.empty() && outputDelayRingBufferSamplesAvailable() == 0) {
#endif
                if (!mEndOfOutput) {
