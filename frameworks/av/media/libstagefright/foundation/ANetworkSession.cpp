...
#define MAX_RTSP_TCP_CONN_RETRY     5
/// @}
...
///@}
     ) 
     {
		...
		///M : Add for Robust issue @{
		///enhance rtsp connect
		if(mode == kModeCreateRTSPClient){
			if(mTestMode){
				
				ALOGE("Run in Sigma Test Mode connect to RTSP server ...");
				int retries = 0;
				err = MakeSocketBlocking(s);
				if (err != OK) {
				    goto bail2;
				}
				while (1)
				{
					...
					if (retries < 3){						
						ALOGE("Rtsp connect fail, retrying after 200ms ...");
						usleep(200 * 1000);
						}else{
							ALOGE("Rtsp connect fail, retrying after 1s ...");
							sleep(1);
						}
					}				
