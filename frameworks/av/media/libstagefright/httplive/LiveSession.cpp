...
#ifdef MTK_AOSP_ENHANCEMENT
sp<ABuffer> LiveSession::createFormatChangeBuffer(bool swap,uint32_t otherStream) {
		ALOGI("createFormatChangeBuffer,swap(%d),otherStream(%d)",swap,otherStream);
#else
...
#ifdef MTK_AOSP_ENHANCEMENT
    //if audio/video uri are diffirent,this way does not work.
    discontinuity->meta()->setInt32("otherStreamFormatAlsoChange", otherStream);
#endif
    ...
    if (mBuffering[idx]) {
        if (mSwitchInProgress
		  ...
		  ALOGD("mask = %d, %d, %d", mask, mStreamMask, mNewStreamMask);
                //if another duration >10s or  this duration > 30s, then set mBuffering[idx] = false;
                //avoid othersource has no media.
                if ((otherSource== NULL) || (otherSource->getEstimatedDurationUs() > 10000000ll)
...
#ifdef MTK_AOSP_ENHANCEMENT
                    int32_t otherStreamFormatAlsoChange = 0;
                    (*accessUnit)->meta()->findInt32("otherStreamFormatAlsoChange", &otherStreamFormatAlsoChange);
                    if(otherStreamFormatAlsoChange > 0) {
                        ALOGD("swapsourceAndCheck for the other stream");
                        swapsourceAndCheck((StreamType)otherStreamFormatAlsoChange,switchGeneration);
                    }
                    swapsourceAndCheck(stream,switchGeneration);
#else
...
#ifdef MTK_AOSP_ENHANCEMENT
			ALOGD("bandwidth estimated at %.2f kbps,according HTTPDataSource", bandwidthBps / 1024.0f);
#else
            ALOGV("bandwidth estimated at %.2f kbps", bandwidthBps / 1024.0f);
#endif 
        } else {
#ifdef MTK_AOSP_ENHANCEMENT
			ALOGW("no bandwidth estimate.");
#else
...
void LiveSession::scheduleCheckBandwidthEvent_l() {
    scheduleCheckBandwidthEvent();
}

void LiveSession::cancelCheckBandwidthEvent_l() {
    cancelCheckBandwidthEvent();
}

bool LiveSession::canSwitchBandwidthTo_l() {
    size_t bandwidthIndex = getBandwidthIndex();
    return canSwitchBandwidthTo(bandwidthIndex);
}

int64_t LiveSession::getFileSize(
        const char *url, int64_t range_offset, int64_t range_length,
...
ssize_t LiveSession::pickBandwidthIndex(int32_t bandwidthBps) {
#if 0
    ssize_t idx = -1;
    idx = (mCurBandwidthIndex + 1) % (mBandwidthItems.size());
    return idx;
#endif
    ssize_t index = -1;
    //bandwidth decrease
    if(mBandwidthItems.itemAt(mCurBandwidthIndex).mBandwidth > (size_t)bandwidthBps) {
    ...
    //bandwidth increase
    else if(mBandwidthItems.itemAt(mCurBandwidthIndex).mBandwidth < (size_t)bandwidthBps) {
        index = mBandwidthItems.size() - 1;
...
void LiveSession::swapsourceAndCheck(StreamType stream,int32_t switchGeneration) {
    swapPacketSource(stream);
    sp<AMessage> msg = new AMessage(kWhatSwapped, id());
    msg->setInt32("stream", stream);
    msg->setInt32("switchGeneration", switchGeneration);
