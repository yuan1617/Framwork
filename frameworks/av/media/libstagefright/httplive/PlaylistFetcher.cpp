...
#ifdef MTK_AOSP_ENHANCEMENT
      mSignalDiscontinuityNoneFlag = false;
      mAbsentStreamMask = 0;
...
status_t PlaylistFetcher::onResumeUntil(const sp<AMessage> &msg) {
    ...
    if (stop) {
        for (size_t i = 0; i < mPacketSources.size(); i++) {
#ifdef MTK_AOSP_ENHANCEMENT
			ALOGD("createFormatChangeBuffer,mAbsentStreamMask =0x%x",mAbsentStreamMask);
			mPacketSources.valueAt(i)->queueAccessUnit(mSession->createFormatChangeBuffer(true,mAbsentStreamMask));
#else
...
void PlaylistFetcher::onDownloadNext() {
    ...
    if (!(mPlaylist->isComplete() || mPlaylist->isEvent()))
        mSignalDiscontinuityNoneFlag = true;

    int64_t fileSize = mSession->getFileSize(uri.c_str(),range_offset, range_length, &source);
    ...
    do {
#ifdef MTK_AOSP_ENHANCEMENT
        int64_t fetchCheckTimeUs = ALooper::GetNowUs();
        int64_t elapseUs = fetchCheckTimeUs - fetchStartTimeUs;
        if(fileSize > 0 && elapseUs > 2500000ll) {
            int64_t estimateBps = buffer->size() * 8E6 / elapseUs;
            ALOGD("estimateBps \t kbps = %" PRId64 " us,according fetchFile speed",estimateBps / 1000ll);
            //terminate this fetching due to bandwidth decrease
            //1.download cost > duration;
            //2.canSwitchBandwidthTo_l

            int64_t itemDurationUs = getSegmentDurationUs(mSeqNumber);
            if ((fileSize - (int64_t)(buffer->size()) > (estimateBps/8 * itemDurationUs)) && mSession->canSwitchBandwidthTo_l()) {
                cancelMonitorQueue();
                mSession->cancelCheckBandwidthEvent_l();
                mSession->scheduleCheckBandwidthEvent_l();
                ALOGD("cancel download due to bandwidth decrease");
                return;
            }
            fetchStartTimeUs = ALooper::GetNowUs();
        }
#endif        
...
#ifdef MTK_AOSP_ENHANCEMENT
    else if (!(mPlaylist->isComplete() || mPlaylist->isEvent()) && mSignalDiscontinuityNoneFlag) {
        //reset the continuity count in ATSParser for live streaming
        //signal discontinuity_none only once per ts file
        //fengyunzhibo.apk
        mTSParser->signalDiscontinuity(ATSParser::DISCONTINUITY_NONE,NULL /* extra */);
        mSignalDiscontinuityNoneFlag = false;
    }
#endif
...
int64_t PlaylistFetcher::getSegmentDurationUs(int32_t seqNumber) const {
    ...
    sp<AMessage> itemMeta;
    CHECK(mPlaylist->itemAt(
                seqNumber - firstSeqNumberInPlaylist, NULL /* uri */, &itemMeta));
