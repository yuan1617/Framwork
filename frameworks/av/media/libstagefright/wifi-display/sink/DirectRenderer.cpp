...
*/
    ...
    /* use drop audio data to replace time stretch function, avoid Source burst send lots of data in */
    if (TotaldelayUs > 100000){
    ...
    ALOGD("kWhatPushAudio delay post=%lld size=%d", delayUs/10, mInputBuffers.size());
    if (delayUs/2>1000)
        (new AMessage(kWhatPushAudio, id()))->post(1000);
    else
        (new AMessage(kWhatPushAudio, id()))->post(delayUs / 10);
#else
