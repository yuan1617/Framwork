...
///@}
        ...
        char buffer [10];
        FILE *fp;
        sprintf(buffer, "%d", serverRtp);
        ALOGI("serverRtp port=%s", buffer);
        if ( (fp=fopen("/proc/sys/net/ipv4/udp_met_port", "w+")) ) {
            fputs(buffer, fp);
            fclose(fp);
        } else {
            ALOGI("write kernel file failed. errno=%d", errno);
        }

        if ( (fp=fopen("/proc/net/wlan0/met_port", "w+")) ) {
            fputs(buffer, fp);
            fclose(fp);
        } else {
            ALOGI("write wlan file failed. errno=%d", errno);
        }
#endif
        ///@}
...
status_t WifiDisplaySource::onTeardownRequest(
    ...
    if ( (fp=fopen("/proc/net/wlan0/met_port", "w+")) ) {
        ...
        ALOGI("write wlan0 file failed. errno=%d", errno);
    }
#endif
    ///@}
