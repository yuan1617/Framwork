                                                            ...
                                                            audio_output_flags_t flags)
{
...
#if 0 //def MTK_LOSSLESS_BT_SUPPORT
    //ship testttt
    if(devices & AUDIO_DEVICE_OUT_BLUETOOTH_A2DP){
        SetLosslessBTSupport(true);
    }
#endif
    ...
    if (status == NO_ERROR && outStream != NULL) {
        ...
        if((devices & AUDIO_DEVICE_OUT_BLUETOOTH_A2DP) && !(config->format & AUDIO_FORMAT_PCM_16_BIT) ){
            ...
            //ship testttt
            String8 notok = String8("no");
            ALOGD("openOutput_l LosslessBT audio check device %d %d ", String8(result)==ok, String8(result)==notok);
