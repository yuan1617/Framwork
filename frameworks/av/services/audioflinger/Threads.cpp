...
#else        // set Audio priority to root.
         set_sched_policy(0, SP_SYSTEM);
#endif
#endif
...
#else
            status = AudioSystem::StartOutputSamplerate(mId, track->streamType(), track->sessionId(),track->mSampleRate);
#endif
            mLock.lock();
...
#else
                    AudioSystem::StopOutputSamplerate(mId, track->streamType(), track->sessionId(),track->mSampleRate);
#endif
                                                          ...
                                                          audio_patch_handle_t *handle)
{
    ...
    if (mOutput->audioHwDev->version() >= AUDIO_DEVICE_API_VERSION_3_0) {
        ...
        mAudioFlinger->SetStereoToMonoFlag(mOutputDevice);
#endif
...
status_t AudioFlinger::MixerThread::setTimestretch(const String8& keyValuePairs, int trackName)
{
	...
	status  = OK;
        #else      
        if(value != mTimeStretchRatio ){
            mTimeStretchRatio = value;
            mTimeStretchTrackName = trackName;
            ALOGV("MixerThread::setTimestretch set Mixer parameter %d",mTimeStretchRatio);
        }
       #endif
    ...
    for (size_t i=0 ; i<count ; i++) {
                ...
                !track->isPaused() && !track->isTerminated())
        {
            ...
            mAudioMixer->setParameter(
                name,
                AudioMixer::TRACK,
                AudioMixer::STEREO2MONO, (void *)mAudioFlinger->mSteroToMono);
#endif
...
#endif
    if(mTimeStretchRatio !=0 && name == mTimeStretchTrackName)
        {
            mAudioMixer->setParameter(name, AudioMixer::TRACK, AudioMixer::DO_TIMESTRETCH, (void *)mTimeStretchRatio);
        }
#endif
            // reset retry count
            track->mRetryCount = kMaxTrackRetries;
