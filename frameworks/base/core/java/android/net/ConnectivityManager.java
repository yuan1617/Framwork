...
//    }
    ...
    public int startUsingNetworkFeature(int networkType, String feature) {
        ...
        /** M: pre-check must fail scenario start */
        IBinder b = ServiceManager.getService(Context.TELEPHONY_SERVICE);
        if (b != null) {
            try {
                ITelephony it = ITelephony.Stub.asInterface(b);
                if (networkType == TYPE_MOBILE && !it.hasIccCard()) {
                    Log.d(TAG, "startUsingNetworkFeature but SIM card not inserted");
                    return PhoneConstants.APN_REQUEST_FAILED;
                }
            } catch (RemoteException e) {
                Log.e(TAG, "startUsingNetworkFeature can not get SIM inserted state");
            }
        }

        try {
            if (mService.isAirplaneModeOn()) {
                Log.d(TAG, "startUsingNetworkFeature but airplane mode on");
                return PhoneConstants.APN_REQUEST_FAILED;
            }
        } catch (RemoteException e) {
            Log.e(TAG, "startUsingNetworkFeature cannot get airplane mode settings");
        }
        /** M: pre-check must fail scenario end */
        ...
        NetworkCallback networkCallback = new NetworkCallback() {
            ...
            public void onAvailable(Network network) {
                ...
                if (networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_MMS)) {
                    Log.d(TAG, "setProcessDefaultNetwork:" + network);
