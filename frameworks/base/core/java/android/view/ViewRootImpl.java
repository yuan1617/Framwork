...
public final class ViewRootImpl implements ViewParent,
    ...
    private static final boolean MOVE_RESPONSE_ENHANCE = SystemProperties.getBoolean(
            "viewRoot.MoveResponseEnhance", false);
    ...
    /// M: Last touch event is DOWN
    private boolean mLastDownEvent = false;

    /// M: This touch event is first MOVE event
    private boolean mFirstMoveEvent = false;
    ...
    private void enableHardwareAcceleration(WindowManager.LayoutParams attrs) {
        ...
        if (hardwareAccelerated) {
            if (!HardwareRenderer.isAvailable()) {
                if (DEBUG_DEFAULT) {
                    Xlog.d(TAG, "system do not support hardware acceleration!!!, this = " + this);
                }
                return;
            }
    ...
    void scheduleTraversals() {
        if (!mTraversalScheduled) {
            /// M : To improve respose time, 1st move do not wait vsync to do performtraversal
            boolean scheduleByHandler = false;
            if (((mLastDownEvent && !mFirstMoveEvent) || mFirstMoveEvent) &&
                    MOVE_RESPONSE_ENHANCE) {
                //Xlog.v(TAG, "scheduleTraversals: by handler ,this = " + this);
                mHandler.post(mTraversalRunnable);
                scheduleByHandler = true;
                if (mFirstMoveEvent) {
                    mLastDownEvent = false;
                }
            }
...
//                        mAttachInfo.mHardwareRenderer.flushLayerUpdates();
    ...
    private void deliverInputEvent(QueuedInputEvent q) {
        ...
        /// M : To improve respose time, 1st move do not wait vsync to do performtraversal
        if (q.mEvent instanceof MotionEvent && MOVE_RESPONSE_ENHANCE) {
    ...
    void scheduleConsumeBatchedInputByHandler() {
        if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
            Xlog.v(TAG, "scheduleConsumeBatchedInputByHandler: mConsumeBatchedInputScheduled = "
                    + mConsumeBatchedInputScheduled + ",mPendingInputEventHead = "
                    + mPendingInputEventHead + ", this = " + this);
        }
        if (!mConsumeBatchedInputScheduled) {
            mConsumeBatchedInputScheduled = true;
            mHandler.post(mConsumedBatchedInputRunnable2);
        }
    }
    ...
    /// M : do not wait for vsync for move event to improve response time
    final class ConsumeBatchedInputRunnable2 implements Runnable {
        @Override
        public void run() {
            doConsumeBatchedInput(System.nanoTime());
        }
    }
    final ConsumeBatchedInputRunnable2 mConsumedBatchedInputRunnable2 =
            new ConsumeBatchedInputRunnable2();
