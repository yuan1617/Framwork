...
public:
    ...
    virtual status_t replay(OpenGLRenderer& renderer, Rect& dirty, int index) {
        ...
        if (g_HWUI_debug_display_list) {
            ...
            dump.appendFormat("%s%d ", index > 0 ? "D" : "M", mOps.size());
        }
#endif

        status_t status = DrawGlInfo::kStatusDone;
        DisplayListLogBuffer& logBuffer = DisplayListLogBuffer::getInstance();
        ATRACE_BEGIN_L2(index > 0 ? "DrawBatch" : "MergingDrawBatch");
        for (unsigned int i = 0; i < mOps.size(); i++) {
            DrawOp* op = mOps[i].op;
...
public:
    ...
    virtual status_t replay(OpenGLRenderer& renderer, Rect& dirty, int index) {
        ...
        renderer.eventMark(op->name());
        ATRACE_BEGIN_L2("MergingDrawBatch");
        nsecs_t duration;
        status_t status;
...
public:
    ...
    virtual status_t replay(OpenGLRenderer& renderer, Rect& dirty, int index) {
        ...
        ATRACE_BEGIN_L2("StateOpBatch");
        nsecs_t duration;
        TIME_LOG_SYSTRACE(const_cast<StateOp *>(mOp)->name(), mOp->applyState(renderer, saveCount), duration, name.string());
        ATRACE_END_L2();
        renderer.endMark();
        TT_END_MARK();
...
public:
    ...
    virtual status_t replay(OpenGLRenderer& renderer, Rect& dirty, int index) {
        ...
        renderer.startMark(name.string());
        renderer.eventMark(const_cast<StateOp *>(mOp)->name());
        ATRACE_BEGIN_L2("RestoreToCountBatch");
        nsecs_t duration;
        TIME_LOG_SYSTRACE(const_cast<StateOp *>(mOp)->name(), renderer.restoreToCount(mRestoreCount), duration, name.string());
        ATRACE_END_L2();
        renderer.endMark();
        TT_END_MARK();
...
status_t DeferredDisplayList::flush(OpenGLRenderer& renderer, Rect& dirty) {
    ...
    ATRACE_BEGIN_L2("fontRenderer endPrecaching");
