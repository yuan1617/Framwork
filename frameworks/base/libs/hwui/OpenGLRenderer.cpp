...
void OpenGLRenderer::eventMark(const char* name) const {
    ...
    if (g_HWUI_debug_display_ops_as_events) TIME_LOG("glInsertEventMarkerExt", mCaches.eventMark(0, name));
#endif
}

void OpenGLRenderer::startMark(const char* name) const {
#if DEBUG_DISPLAY_LIST_OPS_AS_EVENTS
    if (g_HWUI_debug_display_ops_as_events) TIME_LOG("glPushGroupMarkerExt",mCaches.startMark(0, name));
#endif
}

void OpenGLRenderer::endMark() const {
#if DEBUG_DISPLAY_LIST_OPS_AS_EVENTS
    if (g_HWUI_debug_display_ops_as_events) TIME_LOG("glPopGroupMarkerExt",mCaches.endMark());
#endif
}
...
void OpenGLRenderer::countOverdraw() {
    ...
    /// M: Overdraw info on MET
    char str[100];
    sprintf(str, "Overdraw ratio:%f,w:%d,h:%d", mOverdraw, getWidth(), getHeight());
...
status_t OpenGLRenderer::drawRects(Vertex* vertices, int count, const SkPaint* paint) {
    ATRACE_NAME_L2("drawRectsVertex");
