...
/// M: get program binaries from AssetAtlas
...
int ProgramCache::createPrograms(int64_t* map, int* mapLength) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (success) {
            ...
            ALOGD("%d Program id %d, key 0x%.8x%.8x, offset %lld, binaryLength %d within %dns",
...
Program* ProgramCache::generateProgram(const ProgramDescription& description, programid key) {
    if (g_HWUI_debug_enhancement) {
        /// M: enhancement which creating program by binary
        AssetAtlas::ProgramEntry* entry = Caches::getInstance().assetAtlas.getProgramEntry(key);
        if (entry) {
