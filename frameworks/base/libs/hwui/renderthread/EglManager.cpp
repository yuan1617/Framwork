...
void EglManager::initialize() {
    ...
    /// M: initialize program atlas as well
    initProgramAtlas();
}
...
void EglManager::setProgramAtlas(int fd, int length, int64_t* programMap, int programCount) {
    // Already initialized
    if (mProgramFd != -1) {
        ALOGW("Multiple calls to setProgramAtlas!");
        delete programMap;
        return;
    }
    ...
    if (hasEglContext()) {
        // initilizing program atlas doesn't need gl context
        // so pbuffer surface is not needed here
        initProgramAtlas();
    }
}

void EglManager::initProgramAtlas() {
    if (mProgramFd != -1) {
        Caches::getInstance().assetAtlas.initProgramAtlas(mProgramFd, mProgramLength, mProgramMap, mProgramCount);
    }
}
