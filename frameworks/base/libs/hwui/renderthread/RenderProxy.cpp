...
CREATE_BRIDGE5(setProgramAtlas, RenderThread* thread, int fd, int length, int64_t* programMap, int programCount) {
    CanvasContext::setProgramAtlas(*args->thread, args->fd, args->length, args->programMap, args->programCount);
    return NULL;
}

void RenderProxy::setProgramAtlas(int fd, int length, int64_t* programMap, int programCount) {
    SETUP_TASK(setProgramAtlas);
    args->thread = &mRenderThread;
    args->fd = fd;
    args->length = length;
    args->programMap = programMap;
    args->programCount = programCount;
    post(task);
}
