    ...
    private void filterVoiceInputKeyEvent(KeyEvent keyEvent, boolean needWakeLock) {
        ...
        synchronized (mVoiceEventLock) {
            if (keyAction == KeyEvent.ACTION_DOWN) {
				...
				mContext.sendBroadcast(new Intent("com.thunderst.radio.RadioService.PAUSE"));
			} else {
		   ...
		   if(eventtime -mPresstime < 500) {
			mPresstime = 0;
			Intent mIntent = new Intent("com.android.music.musicservicecommand");
                    mIntent.putExtra("command", "next");
			if(mAudioManager != null && mAudioManager.isMusicActive()) {
                    mContext.sendBroadcast(mIntent);
			}
		   } else {
			mPresstime = eventtime;
		   }
                if (mVoiceButtonDown) {
                    // voice button up
