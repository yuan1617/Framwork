    ...
    private static final class AsyncRenderer implements ServiceConnection {
        ...
        public void open(final ParcelFileDescriptor source, final Runnable callback) {
            ...
            new AsyncTask<Void, Void, Integer>() {
                ...
                protected void onPreExecute() {
                	///M: No need to bind service when it`s bound @{
                	if (!mBoundToService) {
        ...
        public void destroy(final Runnable callback) {
            new AsyncTask<Void, Void, Void>() {
                ...
                public void onPostExecute(Void result) {
                	if (mBoundToService) {
                        //FIXME: Placing false at this point is not a good choice,
                        //             it should be done within <code>onServiceDisconnected</code>
                        mBoundToService = false;
                        ///M : In case <code>PDFRenderService</code> has been killed
                        //       by LMK, and this method is executed ahead of the death recipient.
                        //       Since service has been destroyed, nothing will be impacted here,
                        //       just complete the finalizations left. @{
                        try {
                            mContext.unbindService(AsyncRenderer.this);
                        } catch (IllegalArgumentException e) {
                            //FIXME: Maybe more finalization should be done
                            Log.e(LOG_TAG, "Service has been killed.");
                        }
                        ///M : @}
                    }
                    mPageContentCache.invalidate();
