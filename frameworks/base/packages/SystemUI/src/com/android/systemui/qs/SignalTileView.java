...
public final class SignalTileView extends QSTileView {
    ...
    protected View createIcon() {
        ...
        mIconFrame.addView(mRoaming, LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
        return mIconFrame;
    }
    ...
    protected void handleStateChanged(QSTile.State state) {
        ...
        if (s.overlayIconId > 0) {
            ...
            mRoaming.setVisibility(INVISIBLE);
        }
        ...
        if (s.isroaming) {
        	if (s.isConnected) {
                mOverlay.setVisibility(VISIBLE);
                mOverlay.setImageResource(s.overlayIconId);
            	mRoaming.setVisibility(VISIBLE);
            	mRoaming.setPaddingRelative(mIconFrame.getMeasuredWidth()/2 - 8, 2, 0, 0);
            	mRoaming.setImageResource(R.drawable.stat_sys_data_fully_connected_roam);
        	} else {
                mOverlay.setVisibility(VISIBLE);
                mOverlay.setImageResource(R.drawable.ic_qs_signal_r);
                mRoaming.setVisibility(GONE);
        	}
        }else {
        	mRoaming.setVisibility(GONE);
        }
