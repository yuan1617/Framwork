...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        /// M: Support "Default SIM Indicator" : AirplaneMode for Statusbar SimIndicator.
        updateAirplaneMode();
        ...
        mSimIndicatorIcon = (ImageView) mStatusBarView.findViewById(
            R.id.sim_indicator_internet_or_alwaysask);
        ...
        mBatteryController.addLabelView((TextView) mStatusBarWindow.findViewById(R.id.percentage));//yuanluo
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    public void showSimIndicator(String businessType) {
        ...
        Xlog.d(TAG, "showSimIndicator, show SIM indicator which business is " + 
			businessType + "  simId = "+simId+".");
        if (simId == android.provider.Settings.System.DEFAULT_SIM_SETTING_ALWAYS_ASK) {
            List<SubInfoRecord> simInfos = SIMHelper.getSIMInfoList(mContext);
            ...
            int slot = 0;
            for (SubInfoRecord info : simInfos){
                if (info.slotId == simId) {
                    slot = info.slotId;
                    break;
                }
            }
    ...
    public void hideSimIndicator() {
        ...
        for (int i=PhoneConstants.SIM_ID_1; i < SIMHelper.getSlotCount(); i++) {
    ...
    private void updateAirplaneMode() {
        ...
        if (mSimIndicatorIcon != null) {
            mSimIndicatorIcon.setVisibility(mSimIndicatorIconShow && !mAirplaneMode ?
				View.VISIBLE : View.GONE);
        }
    }

    private void updateSimIndicator() {
        Xlog.d(TAG, "updateSimIndicator mIsSimIndicatorShowing = " + mIsSimIndicatorShowing +
			" mBusinessType is " + mBusinessType);
        ...
        if (mSimIndicatorIconShow && mBusinessType != null) {
            long simId = SIMHelper.getDefaultSIM(mContext, mBusinessType);
            if (mSimIndicatorIcon != null
				    && simId != android.provider.Settings.System.DEFAULT_SIM_SETTING_ALWAYS_ASK
                    && simId != android.provider.Settings.System.VOICE_CALL_SIM_SETTING_INTERNET
