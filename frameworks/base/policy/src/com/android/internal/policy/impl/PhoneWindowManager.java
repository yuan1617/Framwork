...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private static final String COVER_STATUS_PATH = "/sys/bus/platform/drivers/mhall/hallstate";
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
			...
			if (attrs != null && (null != attrs.packageName) && (attrs.packageName.startsWith("com.android.ServiceMenu"))){
	               return 0;
	        }
			if (SystemProperties.getBoolean("sys.requireKey", false)) {
				return 0;
			}
        ...
        if (SystemProperties.get("ro.mtk_smartbook_support").equals("1") && mSmartBookPlugIn) {
            if (down && repeatCount == 0 && keyCode == KeyEvent.KEYCODE_WIFI_TOGGLE) {
                ...
                if (!mKeyguardDelegate.isShowingAndNotOccluded()) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
    	...
    	Log.i("yuanluo", "-----keyCode-----" + keyCode);

		if(!SystemProperties.getBoolean("sys.requireKey", false)) {
	    	if (38 == keyCode) {//cover
	    		ContentResolver cr = mContext.getContentResolver();
	            Settings.Global.putLong(cr, "smart_cover_status", System.currentTimeMillis());

	            if (mPowerManager.isScreenOn()) {
	            	Log.i("yuanluo", "----turn off 5041 5047-----");
	    	...
	    	if (36 == keyCode) {
	    		ContentResolver cr = mContext.getContentResolver();
	    		Settings.Global.putLong(cr, "smart_cover_status",
	    				System.currentTimeMillis());
				if (!mPowerManager.isScreenOn()) {
					Log.i("yuanluo", "----turn on-----");
					mPowerManager.wakeUp(SystemClock.uptimeMillis());
				}
				return 0;
	    	}
        ...
        if (32 == keyCode) {// && getCoverStatus().equals("1")
//			ContentResolver cr = mContext.getContentResolver();
//            Settings.Global.putLong(cr, "smart_cover_status", System.currentTimeMillis());
//    		if (android.plugin.Features.JAVA_FEATURE_UI_Style.equals("X1Grand")) {//5037
        	if (true) {
                if (!mPowerManager.isScreenOn()) {
                	Log.i("yuanluo", "----turn on double click & flipcover-----");
                	mPowerManager.wakeUp(SystemClock.uptimeMillis());
                }
    		}
        }
        
//        if (!getCoverStatus().equals("1") && isGestureKey(keyCode)) {
        if (isGestureKey(keyCode)) {
            String status = Settings.System.getString(mContext.getContentResolver(), "gesture_enable");
        	Log.i("yuanluo", "----status-----" + status);
            if (status != null) {
                mEnable = status.charAt(2)== '1' ? true : false;
                Intent intent = new Intent("com.sprocomm.action.SMART_AWAKE");
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent.putExtra("key_code", keyCode);
                if (mEnable) {
                	Log.i("yuanluo", "----isGestureKey-----" + keyCode);
                    mContext.startActivityAsUser(intent, UserHandle.CURRENT);
                }
            } else {
                Intent intent = new Intent();
                intent.setComponent(new ComponentName("com.android.settings", "com.android.settings.SmartAwakeActivity"));
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent.putExtra("key_code", keyCode);
                mContext.startActivityAsUser(intent, UserHandle.CURRENT);
            }
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                {//for alarm snooze and dismiss issue @start
                    Intent intent = new Intent("com.android.alarm.keycode.volume");
                    intent.putExtra("keycode", keyCode);
                    mContext.sendBroadcast(intent);
                }//for alarm snooze and dismiss issue @end
                if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
                    if (down) {
	    ...
	    private boolean isGestureKey(int keyCode) {
    ...
    public static String getCoverStatus() {
        File file = new File(COVER_STATUS_PATH);
