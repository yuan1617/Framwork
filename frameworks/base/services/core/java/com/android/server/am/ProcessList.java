...
import android.util.Log; /// M: LCA: minfree only depends on memory size
import android.view.Display;

/// M: M: LCA: minfree only depends on memory size @ {
/// @}
...
final class ProcessList {
    ...
    private int[] mOomMinFree = new int[mOomAdj.length];
    ...
    private void updateOomLevels(int displayWidth, int displayHeight, boolean write) {
        ...
        if (SystemProperties.get("ro.mtk_lca_ram_optimize").equals("1")) {
            ...
            try {
                ...
                mOomMinFree = new int[adjs.length];
