...
public final class DisplayManagerService extends SystemService {
    ...
    private Runnable mUpdateDisplayState = null;
    /// @}
    ...
    private void handleDisplayDeviceAddedLocked(DisplayDevice device) {
        ...
        DisplayDeviceInfo deviceInfo = device.getDisplayDeviceInfoLocked();
        mUpdateDisplayState = updateDisplayStateLocked(device);
        if (deviceInfo.subtype == DisplayDeviceInfo.EXTRA_TYPE_SMARTBOOK) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (disp.getDisplayIdLocked() == Display.DEFAULT_DISPLAY) {
        ...
        if (mUpdateDisplayState != null &&
                deviceInfo.subtype != DisplayDeviceInfo.EXTRA_TYPE_SMARTBOOK) {
            mUpdateDisplayState.run();
            mUpdateDisplayState = null;
        } else {
            Slog.i(TAG, "Runnable is null or Smartbook plug in, subtype = " + deviceInfo.subtype);
        }
        /// @}
        scheduleTraversalLocked(false);
    }
    ...
    private void updateGlobalDisplayStateLocked(List<Runnable> workQueue) {
        ...
        /// M:[SmartBook]Postpone to unblank SMB @{
        if (SystemProperties.get("ro.mtk_smartbook_support").equals("1")) {
            if (mUpdateDisplayState != null) {
                Slog.d(TAG, "add mUpdateDisplayState");
                workQueue.add(mUpdateDisplayState);
                mUpdateDisplayState = null;
            }
        }
        /// @}
