...
public class AppWindowAnimator {
    ...
    public void clearDeferredThumbnail() {
        if (deferredThumbnail != null) {
            /// M:[ALPS01750731] Fix the thumbnail surface destroyed too early @{
            final int size = mAppToken.allAppWindows.size();
            boolean surfaceStillExist = false;
            for (int i = 0 ; i < size; i++) {
                final WindowState w = mAppToken.allAppWindows.get(i);
                if (w.mHasSurface) {
                    surfaceStillExist = true;
                    break;
                }
            }
            if (!surfaceStillExist) {
