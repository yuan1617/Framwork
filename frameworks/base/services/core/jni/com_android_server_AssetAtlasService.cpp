...
/// M: used to cache program binaries
...
static void com_android_server_AssetAtlasService_release(JNIEnv* env, jobject,
...
#define CLEANUP_GL_MEMORY_AND_RETURN(result) \
    if (needDelete) cache.loadProgramBinariesAndDelete(map, mapArrayLength, binaryBuffer, memoryLength); \
    if (fd >= 0) close(fd); \
    env->ReleaseLongArrayElements(programMap, map, 0); \
    CLEANUP_GL_AND_RETURN(result);

static jboolean com_android_server_AssetAtlasService_build(JNIEnv* env, jobject,
        jobject fileDescriptor, jintArray bufferLength, jlongArray programMap, jintArray mapLength) {
    ...
    int mapArrayLength = env->GetArrayLength(programMap);
    jlong* map = env->GetLongArrayElements(programMap, NULL);
    int memoryLength = cache.createPrograms(map, &mapArrayLength);
    ...
    fd = ashmem_create_region("AssetAtlasProgramCache", memoryLength);
    if (fd < 0) {
        jniThrowException(env, "java/io/IOException", "ashmem_create_region failed");
        CLEANUP_GL_MEMORY_AND_RETURN(false);
    }

    // mmap the shared memory to this process
    // and start to cache program binaries in the buffer
    binaryBuffer = (void*)mmap(NULL, memoryLength, PROT_WRITE, MAP_SHARED, fd, 0);
    if (!binaryBuffer) {
        jniThrowException(env, "java/io/IOException", "mmap failed");
        CLEANUP_GL_MEMORY_AND_RETURN(false);
    } else {
        cache.loadProgramBinariesAndDelete(map, mapArrayLength, binaryBuffer, memoryLength);
        needDelete = false;
        ...
        if (result < 0) {
            ...
            jint* ml = env->GetIntArrayElements(mapLength, NULL);
            jint* bl = env->GetIntArrayElements(bufferLength, NULL);
            ml[0] = mapArrayLength;
            bl[0] = memoryLength;

            env->ReleaseIntArrayElements(mapLength, ml, 0);
            env->ReleaseIntArrayElements(bufferLength, bl, 0);
            env->ReleaseLongArrayElements(programMap, map, 0);
...
static JNINativeMethod gMethods[] = {
    ...
    { "nReleaseFileDescriptor", "(Ljava/io/FileDescriptor;)V",
            (void*) com_android_server_AssetAtlasService_release },
    { "nBuildProgramBinaries", "(Ljava/io/FileDescriptor;[I[J[I)Z",
                (void*) com_android_server_AssetAtlasService_build },
    /// @}
};
