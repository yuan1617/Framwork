...
WeakResourceFilter::parse(const String8& str)
{
    ...
    size_t M = 0; /// M: Remove -swxxxdp resources
    mConfigs.clear();
    mConfigMask = 0;
    mConfigs.resize(N);
    for (size_t i = 0; i < N; i++) {
        ...
        /// M: Remove -swxxxdp resources @{
        } else if (part[0] == '-' && part[1] == 's' && part[2] == 'w') {
            const char* x = part+3;
            while (*x >= '0' && *x <= '9') x++;
            if (x == (part+3) || x[0] != 'd' || x[1] != 'p' || x[2] != 0) {
                fprintf(stderr, "Invalid configuration: %s\n", part.string());
                return UNKNOWN_ERROR;
            }
            String8 xName(part+3, x-(part+3));
            mContainsNegativeSmallestScreenWidthDp = (uint16_t)atoi(xName.string());
            //printf("WeakResourceFilter mContainsNegativeSmallestScreenWidthDp: %d\n", mContainsNegativeSmallestScreenWidthDp);
            //printf("str: %s\n", str.string());
            M++;            
            mConfigs.resize(N-M);
        ...
        std::pair<ConfigDescription, uint32_t>& entry = mConfigs.editItemAt(i-M); /// M: Remove -swxxxdp resources
...
{
    /// M: Remove -swxxxdp resources @{
	// For swxxxdp and -swyyydp config, we skip swxxxdp if xxx > yyy.
    if (config.smallestScreenWidthDp > 0) {
        if (mContainsNegativeSmallestScreenWidthDp > 0 &&
            config.smallestScreenWidthDp >= mContainsNegativeSmallestScreenWidthDp) {
            return false;
        }
    }
    /// @}
