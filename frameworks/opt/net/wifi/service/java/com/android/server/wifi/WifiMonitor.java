...
public class WifiMonitor {
   ...
   /* Network connection completed */
    ...
    private static int p2pOperFreq = -1;
    ...
    private P2pStatus p2pError_GroupRemove(String dataString) {
        ...
        if (P2pStatus.NO_COMMON_CHANNEL == err) {
            /* Handle the special case of reason=FREQ_CONFLICT peer_oper_freq=2412*/
            if (4 < tokens.length) {
                String[] nameValueTwo = tokens[4].split("=");

                if (2 != nameValueTwo.length) {
                    Log.e(TAG, "peer_oper_freq format is wrong-1 @p2pError_GroupRemove() ==> " + tokens[4]);
                    return P2pStatus.NO_COMMON_CHANNEL;
                }
                if (!tokens[4].substring(0,14).equals("peer_oper_freq")) {
                    Log.e(TAG, "peer_oper_freq format is wrong-2 @p2pError_GroupRemove() ==> " + tokens[4]);
                    return P2pStatus.NO_COMMON_CHANNEL;
                }
                if (null == nameValueTwo[1]) {
                    Log.e(TAG, "peer_oper_freq format is wrong-3 @p2pError_GroupRemove() ==> " + tokens[4]);
                    return P2pStatus.NO_COMMON_CHANNEL;
                }
                
                p2pOperFreq = Integer.valueOf(nameValueTwo[1]);
                Log.d(TAG, "p2pOperFreq@p2pError_GroupRemove() = " + p2pOperFreq);
                return P2pStatus.NO_COMMON_CHANNEL;
            }               
            return P2pStatus.NO_COMMON_CHANNEL;
        }
    ...
    private P2pStatus p2pErrorEx(String dataString) {
        ...
        if (P2pStatus.NO_COMMON_CHANNEL == err) {
            /* Handle the special case of reason=FREQ_CONFLICT peer_oper_freq=2412*/
            if (2 < tokens.length) {
                String[] nameValueTwo = tokens[2].split("=");

                if (2 != nameValueTwo.length) {
                    Log.e(TAG, "peer_oper_freq format is wrong-1 @p2pErrorEx() ==> " + tokens[2]);
                    return P2pStatus.NO_COMMON_CHANNEL;
                }
                if (!tokens[2].substring(0,14).equals("peer_oper_freq")) {
                    Log.e(TAG, "peer_oper_freq format is wrong-2 @p2pErrorEx() ==> " + tokens[2]);
                    return P2pStatus.NO_COMMON_CHANNEL;
                }
                if (null == nameValueTwo[1]) {
                    Log.e(TAG, "peer_oper_freq format is wrong-3 @p2pErrorEx() ==> " + tokens[2]);
                    return P2pStatus.NO_COMMON_CHANNEL;
                }

                p2pOperFreq = Integer.valueOf(nameValueTwo[1]);
                Log.d(TAG, "p2pOperFreq@p2pErrorEx() = " + p2pOperFreq);
                return P2pStatus.NO_COMMON_CHANNEL;
            }
            return P2pStatus.NO_COMMON_CHANNEL;
        }
    ...
    private void handleP2pEvents(String dataString) {
        if (dataString.startsWith(P2P_DEVICE_FOUND_STR)) {
            ...
            mStateMachine.sendMessage(P2P_GO_NEGOTIATION_FAILURE_EVENT, p2pOperFreq, 0, p2pStatus);
            p2pOperFreq = -1;
            /** @} */
            ...
            P2pStatus p2pStatus = p2pErrorEx(dataString);
            mStateMachine.sendMessage(P2P_GROUP_FORMATION_FAILURE_EVENT, p2pOperFreq, 0, p2pStatus);
            p2pOperFreq = -1;
            /** @} */
            ...
            mStateMachine.sendMessage(P2P_GROUP_REMOVED_EVENT, p2pOperFreq, 0, p2pStatus);
            p2pOperFreq = -1;
            /** @} */
            ...
            P2pStatus p2pStatus = p2pErrorEx(dataString);
            mStateMachine.sendMessage(P2P_INVITATION_RESULT_EVENT, p2pOperFreq, 0, p2pStatus);
            p2pOperFreq = -1;
            /** @} */
        ...
        ///M:@{
            ...
            /* CTRL-EVENT-DISCONNECTED bssid=02:08:22:0e:e2:00 reason=99 locally_generated=1 peer_oper_freq=2412 */
            if (4 < tokens.length &&
                    tokens[4].substring(0,14).equals("peer_oper_freq")) {
                String[] nameValueTwo = tokens[4].split("=");
                if (null != nameValueTwo[1]) {
                    p2pOperFreq = Integer.valueOf(nameValueTwo[1]);
                    Log.d(TAG, "p2pOperFreq@CTRL-EVENT-DISCONNECTED-1 = " + p2pOperFreq);
                }
            }
            /* CTRL-EVENT-DISCONNECTED bssid=02:08:22:0e:e2:00 reason=99 peer_oper_freq=2412 */
            if (3 < tokens.length &&
                    tokens[3].substring(0,14).equals("peer_oper_freq")) {
                String[] nameValueThree = tokens[3].split("=");
                if (null != nameValueThree[1]) {
                    p2pOperFreq = Integer.valueOf(nameValueThree[1]);
                    Log.d(TAG, "p2pOperFreq@CTRL-EVENT-DISCONNECTED-2 = " + p2pOperFreq);
                }
            }
            //mStateMachine.sendMessage(P2P_PEER_DISCONNECT_EVENT, nameValue[1]);
            mStateMachine.sendMessage(P2P_PEER_DISCONNECT_EVENT, p2pOperFreq, 0, nameValue[1]);
            p2pOperFreq = -1;
            /** @} */
        /** M: NFC Float II @{ */
