...
public class WifiStateMachine extends StateMachine {
    ...
    */
    ...
    private static final int WIFI_INDIDE_SOURCE = -4;
  ...
  //      Xlog.d(TAG, "setScanResults, scanResults:" + scanResults);
    ...
    private void fetchRssiLinkSpeedAndFrequencyNative() {
        ...
        Xlog.i(TAG, "fetchRssiLinkSpeedAndFrequencyNative, newRssi:" + newRssi + ", newLinkSpeed:" + newLinkSpeed
    ...
    private void updateCountryCode() {
        ...
        if (!isSimAbsent) {
            /* Disable for telephony API */

            String countryIso = null;
            if (mMtkGeminiSupport) {
                int mode = Settings.System.getInt(
                    mContext.getContentResolver(), Settings.System.MSIM_MODE_SETTING, 0);
                Xlog.d(TAG, "Dual SIM mode:" + mode);
                if (mode == 1) {
                    countryIso = tm.getSimCountryIso(PhoneConstants.SIM_ID_1);
                    Xlog.d(TAG, "SIM1 countryIso:" + countryIso);
                } else if (mode == 2) {
                    countryIso = tm.getSimCountryIso(PhoneConstants.SIM_ID_2);
                    Xlog.d(TAG, "SIM2 countryIso:" + countryIso);
                } else if (mode == 3) {
                    String countryIso1 = tm.getSimCountryIso(PhoneConstants.SIM_ID_1);
                    String countryIso2 = tm.getSimCountryIso(PhoneConstants.SIM_ID_2);
